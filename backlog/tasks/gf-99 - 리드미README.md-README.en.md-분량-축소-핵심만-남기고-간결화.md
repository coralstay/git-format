...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 README.en.md가 삭제되고 README.md의 언어 전환 링크와 📖 목차 섹션도 제거된다
- [ ] #2 README.md가 정확히 세 개의 최상위 섹션(왜 만들었는가/무엇을 만들었는가/훅 생애주기마다 어떤 훅이 동작하는가)만 갖는다 - 기존 독립 헤더 13개는 더 이상 존재하지 않는다
- [ ] #3 "왜 만들었는가" 섹션이 에이전트의 다중 커밋 분석 가능성과 사람의 이해 가능성을 핵심 결론으로 명시한다
- [ ] #4 "무엇을 만들었는가" 섹션에 실제 설치 명령(기존 저장소 적용 + --global)이 남아있다
- [ ] #5 "훅 생애주기" 섹션이 pre-commit/commit-msg/post-commit 순서와 각 훅의 역할, AI 귀속/토큰 트레일러(사유 슬러그 포함)를 표로 담는다
- [ ] #6 반복되던 예시 블록(git log -1 등)이 1~2개로 줄어든다
- [ ] #7 README.md 안의 decision-N/GF-N 참조 링크가 삭제/오손되지 않는다
- [ ] #8 shellcheck -s sh 전체와 bats tests/ 전체가 통과한다
<!-- AC:END -->
