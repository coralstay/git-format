...
setup() {
  make_isolated_repo
  # 이 파일의 여러 케이스가 HOME을 가짜 경로로 바꿔 커밋을 태운다 - 그때
  # commit-msg(GF-109에서 Python이 됨)가 실행되게 하려면 python 버전을 고정해야
  # 한다. 자세한 이유는 helpers/git-format.bash의 asdf_pin_python() 주석 참고.
  asdf_pin_python
}
...
# 이다. GF-98 이전에는 위 GF-96/GF-97 테스트들도 SLUG를 tr '/' '-'로
# 계산해 훅의 버그를 그대로 흉내내고 있었다(둘 다 틀렸으니 우연히
# 일치해 버그를 못 잡았다) - 지금은 모두 이 정확한 알고리즘으로 갱신됨.
